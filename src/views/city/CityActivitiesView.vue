<template lang="pug">
h3.mb-3.mb-md-4 {{ cityName.name }}：觀光活動

SortBar.mb-3.mb-md-4

.row.gy-2.gy-md-4
  .col-12.col-sm-6.col-md-4(
    v-for="activity in pagination.pageData" :key="activity.ActivityID"  )
...
export default {
  ...
  setup () {
    ...
    const api = `v2/Tourism/Activity/${cityId}?%24format=JSON`
    ...
    watch(() => route.params.cityId, async (newCity) => {
      ...
      try {
        ...
        activityList.value = await getData(api)
        pagination.value = handleChangePage(
          activityFilter(route.params.searchKeyword, activityList.value)
        )
        isLoading.value = false
      } catch (error) {
        console.log('fetch error', error)
